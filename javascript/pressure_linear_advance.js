...
const Settings = {
  ...
  spoofPrusa() {
    if (this.firmware !== 'ratos' && !this.expert_mode)
      return "";
    return this.spoof_prusa;
  },
};
...
function genGcode() {
  ...
  let date = new Date();
  let dateString = `${date.getUTCFullYear()}-${String(date.getUTCMonth() + 1).padStart(2, '0')}-${String(date.getUTCDate()).padStart(2, '0')} at ${String(date.getUTCHours()).padStart(2, '0')}:${String(date.getUTCMinutes()).padStart(2, '0')}:${String(date.getUTCSeconds()).padStart(2, '0')} UTC`;

  let regularPreamble = `generated by DepauPressureLinearAdvanceTool 0.1${(config.firmware === 'ratos') ? " in RatOS dialect 0.1" : ""} on ${dateString}`;
  let prusaPreamble = `generated by PrusaSlicer ${config.spoofPrusa()} on ${dateString}`;

  // Start G-code for pattern
  state.pa_script += `; ${config.spoofPrusa() ? prusaPreamble : regularPreamble}

; ### Depau's Pressure Advance / Linear Advance Calibration Tool ###
;
; Original tool by Andrew Ellis [https://ellis3dp.com/Pressure_Linear_Advance_Tool/]
